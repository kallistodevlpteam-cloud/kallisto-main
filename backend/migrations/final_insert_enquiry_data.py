"""Delete existing records and insert ALL remaining data for enquiry projects 16-19.

Run from the backend directory:
    python migrations/final_insert_enquiry_data.py
"""
...
def main() -> None:
    ...
    print(f"Processing remaining data for projects {project_ids}...")

    # Step 1: Delete existing records that may have been partially inserted
    print("\nDeleting partial records...")
    for pid in project_ids:
        ...
        print(f"  Cleared remaining tables for project {pid}")
    ...
    # family_details
    families_data = [
        ("FAM-0016-1", "CLI-0016", "Primary decision maker, works from home 3 days/week, prefers quiet office corner", "2026-08-21", "2026-08-21", "Rahul Nair", 38, "Software Engineer at Infosys", "9876543212", "Husband / Father", None),
        ("FAM-0016-2", "CLI-0016", "Co-decision maker on aesthetics, colors, and materials. Has strong opinions on Vastu compliance", "2026-08-21", "2026-08-21", "Ananya Nair", 35, "Interior Designer (freelance)", "9876543213", "Wife / Mother", None),
        ("FAM-0016-3", "CLI-0016", "Needs study desk by window, loves art and craft, wants pink accent wall", "2026-08-21", "2026-08-21", "Meera Nair", 8, "Student (Grade 3)", None, "Daughter", None),
        ("FAM-0016-4", "CLI-0016", "Needs small play area, loves dinosaurs and cars", "2026-08-21", "2026-08-21", "Aarav Nair", 5, "Kindergarten", None, "Son", None),
        ("FAM-0018-1", "CLI-0018", "Heritage enthusiast, detail-oriented, wants every change documented with photos", "2026-08-21", "2026-08-21", "Dr. Thomas Mathew", 68, "Retired Professor of History", "9876543214", "Owner", None),
        ("FAM-0018-2", "CLI-0018", "Concerned about modern comfort, wants western toilet, geyser, and mixer grinder in kitchen", "2026-08-21", "2026-08-21", "Saramma Mathew", 65, "Homemaker", "9876543215", "Wife", None),
    ]
    ...
    print("\n=== FULL VERIFICATION ===")
    tables_to_check = [
        'projects', 'client_details', 'project_site', 'project_budget', 'enquiry_details',
        'project_clients', 'project_lifestyle', 'project_approval_process', 'project_communication',
        'project_technical', 'project_regulatory', 'project_outdoor', 'project_timeline',
        'project_spaces', 'requirements', 'requirement_items', 'family_details', 'inspiration_img',
        'project_DOC', 'project_scope', 'project_scope_item'
    ]
    
    for table in tables_to_check:
        try:
            r = pipeline([f'SELECT COUNT(*) FROM {table} WHERE project_id IN (16,17,18,19)'])[0]
            count = rows(r)[0][0]
            print(f"  {table}: {count}")
        except Exception as e:
            # Some tables don't have project_id directly
            if table == 'requirement_items':
                r = pipeline(['SELECT COUNT(*) FROM requirement_items WHERE requirement_id IN (SELECT id FROM requirements WHERE project_id IN (16,17,18,19))'])[0]
                count = rows(r)[0][0]
                print(f"  {table}: {count}")
            elif table == 'family_details':
                r = pipeline(["SELECT COUNT(*) FROM family_details WHERE client_id IN (SELECT client_id FROM client_details WHERE project_id IN (16,17,18,19))"])[0]
                count = rows(r)[0][0]
                print(f"  {table}: {count}")
            elif table == 'project_scope_item':
                r = pipeline(['SELECT COUNT(*) FROM project_scope_item WHERE scope_id IN (SELECT id FROM project_scope WHERE project_id IN (16,17,18,19))'])[0]
                count = rows(r)[0][0]
                print(f"  {table}: {count}")
            else:
    ...
    print("\n=== PROJECT DETAILS ===")
    for pid in project_ids:
        r = pipeline(["SELECT project_name, brief_description, sq_area, over_view FROM projects WHERE id = ?"], [[pid]])[0]
        p = rows(r)[0]
        print(f"Project {pid}: {p[0]}")
        print(f"  Area: {p[2]} sq ft | Description: {p[1][:60]}...")
        print(f"  Overview: {p[3][:80]}...")

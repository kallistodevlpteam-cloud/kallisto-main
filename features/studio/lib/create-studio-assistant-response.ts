...
export function createStudioAssistantResponse(
  ...
  const isRevision =
    ...
    lowerPrompt.includes("revise proposal") ||
    lowerPrompt.includes("revise draft") ||
    lowerPrompt.includes("update timeline") ||
    lowerPrompt.includes("change timeline") ||
    lowerPrompt.includes("revise payment terms") ||
    lowerPrompt.includes("update payment terms");
  ...
  const isExplicitProposalCreation =
    ...
    (isInitialOutputGeneration && (agentResolution?.agentId === "proposal" || classification.kind === "actionable" && classification.outputType === "proposal"));
